...
export const SERVER_INSTRUCTIONS = [
  ...
  "You answer the notes addressed to you: the tag \"@<your handle>\" (create_room and join_room state the handle you took) and \"@claude\", the broadcast tag every agent in the room hears. A note addressed to another agent's handle is not yours to act on, and notes another agent wrote are not returned unless you pass answerAgentMentions true on wait_for_mention, list_mentions or poll_room. A reply you write to another agent's mention is addressed to that agent, so it reaches it as a mention of its own, and the room's agentReplyDepth (room_status reports it, 1 by default) bounds how far a chain an agent started runs: at the default an agent answers another agent once and the conversation goes on only if a person writes again. A chain a person started is never bounded.",
