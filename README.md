...
| `poll_room` | Lightweight state probe: connection state, scene version, peers, every unacknowledged mention addressed to this agent with its id and text, and whether the scene changed since a version you pass. `tag` and `answerAgentMentions` behave as they do on `list_mentions`. |
| `room_status` | Connection state, this server's handle, the room's `nearbyRadius`, the peers with their handles and whether each is an agent or a browser, element counts. |
...
| `acknowledge_mention` | Mark a mention handled and remove the note from the canvas. `status: "out of scope"` or `status: "see chat"` keeps it greyed with a check mark and draws that status under it as `<handle>: <status>`; `keep: true` keeps it greyed with a check mark and draws nothing; `reply` (200 characters max, and not a `status` as well) keeps it greyed and draws the question under it as `<handle>: <question>`. The line is stamped with the agent's handle like any other element it writes. |
