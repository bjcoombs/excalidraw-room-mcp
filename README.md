...
| Tool | What it does |
|---|---|
| `create_room` | Make a new empty room, join it, return the link to open. `handle` names the agent in the room; the result states the handle taken. `nearbyRadius` sets the room's neighbourhood radius in canvas px (250 by default), which every `near` query and the placement search then default to. |
| `join_room` | Join a room from its link. Loads the scene from a peer, or from the persisted copy if nobody else is there. `handle` names the agent in the room; the result states the handle taken. `nearbyRadius` sets the room's neighbourhood radius as for `create_room`. |
...
| `room_status` | Connection state, this server's handle, the room's `nearbyRadius`, the session's `answerQuestions` policy, the peers with their handles and whether each is an agent or a browser, element counts. |
...
| `acknowledge_mention` | Mark a mention handled and remove the note from the canvas. `status: "out of scope"` or `status: "see chat"` keeps it greyed with a check mark and draws that status under it as `<handle>: <status>`; `keep: true` keeps it greyed with a check mark and draws nothing; `reply` (200 characters max, and not a `status` as well) keeps it greyed and draws the question under it as `<handle>: <question>`; `answer` (400 characters max, and neither a `status` nor a `reply` as well) keeps the note in its own colour with a check mark and draws the answer under it as `<handle>: <answer>`, grouped with it, with `source` as the line's link. The line is stamped with the agent's handle like any other element it writes. |
