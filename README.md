...
The canvas takes the whole widget. Under it is one status bar: the connection state, the peer count, the element count, how many `@claude` mentions are still pending, when the last update landed, and an **Open in browser** control that hands the room to your browser through the host. A host may refuse to open a link, and then the bar shows the room link as text to copy instead; `open_room` opens the same room from the server side. In a host that does not proxy server tools there is no polling at all: the bar says so and carries a **Refresh** button, which is the only way to update the view there. Everywhere else there is nothing to press.
