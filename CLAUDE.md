...
npm run e2e:show-room -- "<collab link>"   # manual: joins, prints the scene_show payload, exits
...
- `view/` is a separate Vite build (React + `@excalidraw/excalidraw`) producing the single file `dist/view/canvas.html`, the MCP Apps resource the chat host renders. Its dependencies live in the root `package.json`; there is no nested lockfile. `src/view.ts` builds the `scene_show` payload and serves that file.
...
- `.mcpbignore` patterns are gitignore-style, so a pattern naming a directory that also exists under `dist/` must be anchored with a leading slash. `/view/` excludes the Vite sources; unanchored `view/` excluded `dist/view/canvas.html` too and shipped a bundle whose `scene_show` resource 404s. `npm run check:bundle` (`scripts/check-bundle.sh`) is the guard, and the release workflow runs it on the artifact it is about to attach.
- `src/instructions.ts` holds the `instructions` string sent at initialize and the listen tip appended to `room_create` and `room_join`. `agents/canvas-listener.md` is the Sonnet subagent that runs the listen loop and `agents/canvas-answerer.md` the one it spawns per knowledge question; both are documented in README under Collaborating, and `install-agent` copies every file in `agents/`, listed in `BUNDLED_AGENTS` in `src/install-agent.ts`. An agent's `tools:` line is its whole grant, so the answerer's five tools are what stops it drawing; `src/agents.test.ts` asserts that list exactly.
- **The tool surface has a context budget.** `src/budget.test.ts` drives the built server over stdio and fails if a `tools/list` entry reaches 900 characters, the list 10,000, the instructions 1,500, a tool description 300 or an argument description 100. Tool names carry their group (`room_`, `scene_`, `mention_`); a description says when to use the tool and what comes back, nothing more. Formats, rules and rationale go in README, which `room_help` serves by topic: `src/help.ts` maps each topic to README headings and reads `README.md` beside `dist/` at runtime, so renaming a README heading means updating that map (`src/help.test.ts` fails otherwise). `scene_add` emits only the element `type` and checks the full strict spec in its handler, because the declared spec alone is over an entry's budget.
- `src/room.ts` socket + scene state + `waitForMention`; `src/crypto.ts` AES-GCM; `src/reconcile.ts` merge rule; `src/elements.ts` builders + summariser; `src/mentions.ts` `@claude` detection and neighbourhood; `src/firebase.ts` persistence; `src/snapshot.ts` SVG writer + resvg raster for `scene_snapshot`; `src/index.ts` MCP tool surface; `src/e2e.ts` manual driver.
...
- **Never call the Excalidraw imperative API from a render.** `onExcalidrawAPI` hands the instance over during Excalidraw's own render, before its component mounts, and `updateScene` on an unmounted Excalidraw is a silent no-op - React logs "can't call setState on a component that is not yet mounted" and nothing is drawn. `view/src/app.tsx` records the instance in the callback and draws from an effect, which runs after the child has mounted. This is what made the chat widget a still frame in issue #30.
...
- **A host caches the MCP Apps resource HTML by URI.** Claude Desktop 1.49585.0 rendered the 0.5.1 view with extension 0.5.3 installed on disk; its log had read `ui://excalidraw-room/canvas.html` three times on one afternoon and never again after any reinstall. `CANVAS_RESOURCE_URI` in `src/view.ts` therefore carries `PACKAGE_VERSION`, so each release is a new URI and a fresh fetch. Keep it versioned, and prefer `room_open` (excalidraw.com in the default browser) when a person needs to watch the canvas.
- **`room_open` must not block or write to stdout.** `src/open.ts` spawns the platform opener detached with stdio ignored and unrefs the child; a spawn failure is reported in the result text, never thrown. `EXCALIDRAW_ROOM_NO_OPEN=1` returns the link without launching anything, which is how the tests and CI run it.
...
- **Element ids are immutable and unique.** `scene_add` rejects known or repeated ids; `scene_update` preserves the id.

## Testing boundaries

- Unit tests cover crypto, reconcile, elements, the Firestore load/save paths (fetch stubbed), and the link-parsing and pre-join paths of `room.ts`. The socket, broadcast and persist paths of `room.ts`, and all of `index.ts`, are exercised only by the manual e2e driver against a live room. If you change those, run the e2e with a browser open on the link and check both directions (server writes appear on the canvas; a pencil stroke appears in `scene_read`).
