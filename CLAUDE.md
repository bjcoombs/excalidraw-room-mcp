...
- `src/instructions.ts` holds the `instructions` string sent at initialize and the listen tip appended to `room_create` and `room_join`. `agents/canvas-listener.md` is the Sonnet subagent that runs the listen loop; it is documented in README under Collaborating.
